#####################################################################################################################
# A python utility script to perform cell averaging CFAR
#####################################################################################################################
...
# Improved CFAR algorithm which also detects peaks near boundaries
def get_peak_indexes_improved(data, num_train, num_guard, threshold_delta):
    data = data.flatten()
    peak_indexes = []
    range_start = 0
    range_end = data.shape[0]
    num_train_half = int(round(num_train / 2))
    num_guard_half = int(round(num_guard / 2))
    window_size = int((num_train_half + num_guard_half) * 2)
    for index in range(range_start, range_end):
        lower_begin = index - (window_size / 2)
        upper_end = index + (window_size / 2) + 1
        if (lower_begin < range_start):
            lower_begin = range_start
            upper_end   = lower_begin + window_size
        if (upper_end > range_end):
            upper_end = range_end
            lower_begin = upper_end - window_size
        lower_end = index - num_guard_half
        upper_begin = index + num_guard_half + 1
        if (lower_end < range_start):
            lower_end = range_start
        if (upper_begin > upper_end):
            upper_begin = range_end
        lower_begin, lower_end, upper_begin, upper_end = int(lower_begin), int(lower_end), int(upper_begin), int(upper_end)
        train_data = np.hstack([data[lower_begin : lower_end], data[upper_begin : upper_end]])
        average = np.average(train_data)
        if data[index] >= average + threshold_delta:
