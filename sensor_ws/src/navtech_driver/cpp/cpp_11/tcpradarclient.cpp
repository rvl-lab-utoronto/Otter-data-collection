...
namespace
{
	const uint16_t CONNECTION_CHECK_TIMEOUT = 15000;
	const uint16_t READ_TIMEOUT = 60;
	const uint16_t SEND_TIMEOUT = 10;
}
...
bool Navtech::TcpRadarClient::HandleData()
{
	...
	if (bytesRead <= 0 || !_reading || !_running) {
		SetConnectionState(ConnectionState::Disconnected);
		Helpers::Log("TcpRadarClient - Failed to read header");
		return false;
	}
	...
	if (messageHeader.HeaderIsValid() && messageHeader.PayloadLength() != 0) {
		...
		if (bytesTransferred <= 0 || !_reading || !_running) {
			SetConnectionState(ConnectionState::Disconnected);
			Helpers::Log("TcpRadarClient - Failed to read payload");
			return false;
		}
		bytesTransferred++;
