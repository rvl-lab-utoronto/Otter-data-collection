...
{	
	...
	const uint8_t NDMSIGNATUREBYTES[NDMSIGNATURELENGTH] = { 0x00, 0x01, 0x03, 0x03, 0x07, 0x07, 0x0f, 0x0f, 0x1f, 0x1f, 0x3f, 0x3f, 0x7f, 0x7f, 0xfe, 0xfe };
...
#pragma pack(1)
	struct NetworkDataHeader
	{
		...
		void SetPayloadLength(uint32_t  length) {
			payloadsize = htonl(length);
		}
		const uint32_t  PayloadLength() const {
			return ntohl(payloadsize);
		}
		const uint32_t HeaderLength() const {
			return NDMHEADERLENGTH;
		}
		const bool HeaderIsValid() const {
			...
			result &= (version >= NDMPROTOCOLMINVERSION && version <= NDMPROTOCOLVERSION);
		...
		const std::vector<uint8_t> ToData() const {
			...
			std::memcpy(&headerData[NDMPAYLOADSIZEOFFSET], &payloadsize, sizeof(payloadsize));
			return headerData;
		}
	};
#pragma pack(pop)
	...
	public:
		...
		explicit NetworkDataMessage(const NetworkDataHeader header, const uint8_t* payload, const std::size_t size)
			: NetworkDataMessage(header, std::vector<uint8_t>(payload, payload + size)) {}

		explicit NetworkDataMessage(const NetworkDataHeader header, const std::vector<uint8_t>& data)
			: _header(header),
				_payload(data) {}
		...
		const uint32_t PayloadSize() const
		{
			return _header.PayloadLength();
		}
		const uint8_t Version() const {
			return _header.version;
		}
		const bool MessageValid() const
		{
			return _header.HeaderIsValid();
		}
