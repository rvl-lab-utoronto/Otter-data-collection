...
#pragma pack(1)
    ...
    {
        num_blocks = static_cast<std::uint8_t>(count);
    }
    ...
    std::size_t Video::update_video_info(std::size_t data_sz)
    {
        ...
        num_blocks = static_cast<std::uint8_t>(data_sz / block_size()) + ((data_sz % block_size() != 0) ? 1 : 0);

        // We only use 8/16-bit resolution
        //
        // auto cell_sz = uint24_t::overlay_at(reinterpret_cast<std::uint8_t*>(&video_size_cells));
        video_size_cells = static_cast<std::uint32_t>(data_sz / (bit_resolution() / 8));
    ...
    {
        constexpr Unit::Metres_per_sec c    { 299'792'458.0f };
        constexpr float                fsec { 1e15f };

        return static_cast<std::uint32_t>((s / (c / 2)) * fsec); 
    }
    ...
    {
        return static_cast<std::uint16_t>(angle.to_float() * ((1 << 16) / 360.0f));
    }
