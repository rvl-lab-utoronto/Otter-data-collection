...
namespace Navtech {
    ...
    inline Networking::IP_address operator""_ipv4(const char* addr_str, std::size_t sz)
    {
        return Networking::IP_address { std::string_view { addr_str, sz } };
    }


    inline Networking::IP_address operator""_ipv4(unsigned long long raw_addr)
    {
        return Networking::
        IP_address { static_cast<std::uint32_t>(raw_addr) };
    }
