...
namespace Navtech::Networking {
    ...
    IP_address::IP_address(std::uint32_t raw_addr, Endian endian) :
        address { (endian == Endian::host) ? raw_addr : to_uint32_host(raw_addr) }
    {
    }
    ...
    IP_address::IP_address(const sockaddr& socket_addr)
    {
        ...
        address.word = to_uint32_host(static_cast<std::uint32_t>(addr.sa_ipv4.sin_addr.s_addr));
    }


    IP_address::IP_address(const sockaddr_in& socket_addr) :
        address { to_uint32_host(static_cast<std::uint32_t>(socket_addr.sin_addr.s_addr)) }
    {
    }
    ...
    {
        return to_uint32_network(address.word);
    }
    ...
    IP_address::Byte_array IP_address::to_byte_array() const
    {
        Byte_array bytes { };
