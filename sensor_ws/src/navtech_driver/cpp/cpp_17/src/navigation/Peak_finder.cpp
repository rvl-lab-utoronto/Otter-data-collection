...
namespace Navtech::Navigation {
    ...
    std::vector<Navigation::Polar::Target> Azimuth_target::to_polar() const
    {
        std::vector<Navigation::Polar::Target>  polar_targets   { };
        
        for (auto& t : targets) {
            polar_targets.emplace_back(
                static_cast<Unit::Metre>(t.range),
                Unit::Degrees { static_cast<float>(angle) },
                static_cast<Unit::dB>(t.power)
            );
        }

        return polar_targets;
    }
            ...
            if (buffer_mode == Buffer_mode::average) {
                for (auto bin = 0; bin < configuration.range_in_bins; bin++) {
                    ...
                    for (std::uint32_t counter = 0; counter < data_buffer.size(); counter++) {
                        float value { static_cast<float>(data_buffer[counter][bin]) / 2.0f };
                        total += std::pow(10.0f, static_cast<float>(value / 10.0f));
                    }
                ...
                for (std::int32_t bin = 0; bin < configuration.range_in_bins; bin++) {
                    for (std::uint32_t counter = 0; counter < data_buffer.size(); counter++) {
                        double value { data_buffer[counter][bin] / 2.0 };
    ...
    )
    {
        ...
        while (peak_bin != configuration.range_in_bins) {
            ...
            if (peak_bin < configuration.range_in_bins) {
                float resolvedBin { static_cast<float>(peak_resolve(data, peak_bin, bins_to_operate_on)) };
                float range {
